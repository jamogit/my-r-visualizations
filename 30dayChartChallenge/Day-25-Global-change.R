...
df <- df.raw |> filter(Model == "REMIND-MAgPIE" & Damage == "High" & ISO3 == "WLD" & Percentile == 95)
...
ggplot(plotdata, aes(x = Year, y = Value, group = Scenario, colour = Scenario, fill = Scenario)) +
  ...
  ylim(-20, 0) +
  labs(
    title = "Potential National Income Loss from chronic climate damage",
    subtitle = "Damage estimate = High*. Assessment model = REMIND-MAgPIE**",
    x = "Year",
    y = "Change in National Income, %",
    caption = "Source: IMF - Climate Change Dashboard | #30DayChartChallenge - Global change - Day 25 | DataViz: @janmoilanen_ek
    ",
...
ggsave("Day25-Clobal-change-4K.png", units = c("px"), width = 3840, height = 2160)
